function cutOffTree(forest) {
    var goNorth = function (pos, rows, cols) {
        if (pos[0] - 1 < 0) {
            return null;
        }
        return [pos[0] - 1, pos[1]];
    };
    var goEast = function (pos, rows, cols) {
        if (pos[1] + 1 >= rows)
            return null;
        return [pos[0], pos[1] + 1];
    };
    var goSouth = function (pos, rows, cols) {
        if (pos[0] + 1 >= cols)
            return null;
        return [pos[0] + 1, pos[1]];
    };
    var goWest = function (pos, rows, cols) {
        if (pos[1] - 1 < 0)
            return null;
        return [pos[0], pos[1] - 1];
    };
    var checkTrees = function (forest) {
    ...
    var moveChecker = function (moves) {
    ...
    var currPos = [0, 0];
    var currCell = forest[currPos[0]][currPos[1]];
    var steps = 0;
    var maxSteps = (forest.length * forest[0].length) - 1;
    var rows = forest[0].length;
    var cols = forest.length;
    var areStepsAvailable = true;
    var nextMinVal = currCell;
    var nextPos;
    ...
    while (areStepsAvailable) {
        ...
        var northPos = goNorth(currPos, rows, cols);
        var eastPos = goEast(currPos, rows, cols);
        var southPos = goSouth(currPos, rows, cols);
        var westPos = goWest(currPos, rows, cols);
        var NCell = northPos ? forest[northPos[0]][northPos[1]] : null;
        var ECell = eastPos ? forest[eastPos[0]][eastPos[1]] : null;
        var SCell = southPos ? forest[southPos[0]][southPos[1]] : null;
        var WCell = westPos ? forest[westPos[0]][westPos[1]] : null;
        var directions = [NCell, ECell, SCell, WCell];
        // ===== ESTABLISH POSSIBLE STEPS  
